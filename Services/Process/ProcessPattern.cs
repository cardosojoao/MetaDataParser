...
namespace MetaDataParser.Services.Process
{
    ...
    {
        private void WritePatternStorage(StorageGroup storage)
        {
            ...
            string filePathIndexHeader = Path.Combine(rootPath, Path.GetFileNameWithoutExtension(storage.DataTableName) + Path.GetExtension(storage.IndexTableName));
            ...
            foreach (StorageGroupFile file in storage.FileList)
            {
                if (pageSize + file.Length > 8192)
                {
                    ...
                    pageSize = 0;
                    indexData.Append("\n\t\tmmu\t$").Append(storage.Org).Append(", (").Append(storage.InitalBank).Append('+').Append(pageNumber).Append(")\n");
                    indexData.Append("\t\torg\t\t$").AppendLine(storage.Org);
                }
            ...
            File.WriteAllText(filePathIndexHeader, indexDataFirst.ToString());
        }
