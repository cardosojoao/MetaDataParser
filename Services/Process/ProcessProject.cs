...
namespace MetaDataParser.Services.Process
{
    public partial class ProcessProject : IProcessProject
    {
        ...
        public void Process(List<StorageGroup> groups)
        {
            foreach (StorageGroup group in groups)
            {
                switch (group.Type)
                {
                    case "pattern":
                        {
                            WritePatternStorage(group);
                            break;
                        }
                    case "scene":
                        {
                            WriteSceneStorage(group);
                            break;
                        }
                    case "dictionaryfiles":
                        {
                            WriteDictionaryFilesStorage(group);
                            break;
                        }
                    case "sprite":
                        {
                            WriteSpriteStorage(group);
                            break;
                        }
                    case "dictionarybinary":
                        {
                            WriteDictionaryBinnaryStorage(group);
                            break;
                        }
                    default:
                        {
        ...
        private void WriteDictionaryStorage(StorageGroup storage)
        {
